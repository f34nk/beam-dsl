...
decode_event_stream(Body) when is_binary(Body) -> [decode_event_stream_event(F) || F <- aws_event_stream:decode_frames(Body)].

encode_event_stream_event({member, Value}) ->
    Payload = jsone:encode(maps:filter(
        fun
            (_, V) -> (V =/= undefined)
        end,
...
decode_event_stream_event_type(<<"member">>, Payload) -> {member, #member_event{
